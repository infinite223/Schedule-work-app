import { View, Text, Pressable, StyleSheet, Dimensions, TouchableOpacity, TextInput } from 'react-native'
...
const Page = () => {
  const { groupName , groupId, groupDescription }: ParamsTypes = useLocalSearchParams()
  ...
  const tryEditGroup = async () => {
    ...
    if(jsonValue){
      const res = await updateGroup(
        JSON.parse(jsonValue).authToken, name, description, groupId
      ) 

      if(res.status === 200) {
        const updatedGroup = await res.json()
        let newGroups: Group[] = groups.filter((group: Group) => group.id !== updatedGroup.id)
        newGroups.push(updatedGroup)
        dispatch(setGroups(newGroups))

        router.back()
        router.push('/messageModal')
        router.setParams({ message: "Udało się zaktualizować grupę", type: 'SUCCESS' })
      }
      else {
        router.push('/messageModal')
  ...
  const tryRemoveGroup = async () => {
    ...
    if(jsonValue){
      ...
      if(res.status === 200){
        ...
        let newGroups: Group[] = groups.filter((group: Group) => group.id.toString() !== groupId.toString())
        console.log(newGroups, 'nowe grupy')
        dispatch(setGroups(newGroups))

        router.back()
        router.push('/messageModal')
        router.setParams({ message: "Udało się usunąć grupę", type: 'SUCCESS' })
      }
      else {
        router.push('/messageModal')
        router.setParams({ message: "Coś poszło nie tak", type: 'ERROR' })
